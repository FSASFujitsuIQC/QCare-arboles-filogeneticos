...
Other methods have also been compiled here, such as `func_QUBOmod` (divisive method with a QUBO formulation based on modularity), `func_Modularity` (same as NMCutDA but with modulatiry instead of Ncut post-processing) and `func_back_mod` (the first attempt at a bottom-up method with modularity before NJDA). These have all been discared due to poor results and therefore are not updated.

Finally, helper functions are also listed: `func_cleanfasta` (cleans FASTA files that are not properly formatted), `func_filter_nodes` (lists trees and the number of nodes they have), `Representaciones_graficas` (a compilation of all the drafts and code to print the figures present in the deliverable).
